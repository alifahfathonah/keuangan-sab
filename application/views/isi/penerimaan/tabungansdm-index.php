...
<script type="text/javascript">
	jQuery(function($){
		...
		$('#loader-data').show();
		$('#loader-form').show();
		$('#data').load('<?php echo site_url();?>penerimaan/tabungansdmdata',function(){
			$('#loader-data').hide();
		});
		$('#form').load('<?php echo site_url();?>penerimaan/tabungansdmform/-1',function(){
			$('#loader-form').hide();
		});
		$('#siswa').change(function(){
			var id=$(this).val();
			$('#loader-data').show();
			$('#data').load('<?php echo site_url();?>penerimaan/tabungansdmdata/'+id,function(){
				$('#loader-data').hide();
			});
		});
	});
	...
	{
		$('#loader-form').show();
		$('#form').load('<?php echo site_url();?>penerimaan/tabungansdmform/'+id,function(){
			$('#loader-form').hide();
		});
	}
	...
	function hapus(id)
	{
		bootbox.confirm("<h3>Yakin Ingin Menghapus Data Siswa ini ?? </h3>", function(result) {
					...
					{
						$.ajax({
							url : '<?php echo site_url();?>penerimaan/tabungansdmhapus/'+id,
							success : function(a)
							{
								// alert(a);
								tampilpesan(a);
	...
	function detailtabungan(id,norek)
	{
		$.ajax({
			url : '<?php echo site_url();?>penerimaan/tabungansdmdetail/'+id+'/'+norek,
			success : function(a)
			{
				bootbox.alert({
					size : "large",

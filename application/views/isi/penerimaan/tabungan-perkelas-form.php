					...
					<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<select  class="chosen-select" name="kelas" id="siswaa" data-placeholder="Nama Kelas">
									<option value=""></option>
									<?php
...
<script type="text/javascript">
	...
	$('#siswaa').change(function(){
		var id=$(this).val();
		$('#data').load('<?=site_url()?>penerimaan/tabunganperkelasdataform/'+id);
	});
	...
	$("#simpan").on(ace.click_event, function() {
		...
		if(siswa=='')
		{
			...
			$('#pesansiswa').text('Nama Kelas Belum Dipilih');
		...
		else
		{
				bootbox.confirm("<h3>Apakah Data Tabungan Siswa ini sudah Benar </h3>", function(result) {
					...
					{
						$.ajax({
							url : '<?=site_url()?>penerimaan/tabunganperkelasproses/<?=$id?>',
							type : 'POST',
							data : $('#simpantabungankelas').serialize(),
							success : function(a)
							{
								// alert(a);
								tampilpesan(a);
								var idkelas=$('#siswaa').val();
								$('#data').load('<?=site_url()?>penerimaan/tabunganperkelasdata/'+idkelas);

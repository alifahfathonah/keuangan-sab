...
<script type="text/javascript">
	jQuery(function($){
		$('#loader-data').show();
		$('#loader-form').show();
		$('#data').load('<?=site_url()?>penerimaan/jenisdata',function(){
			$('#loader-data').hide();
		});
		$('#form').load('<?=site_url()?>penerimaan/jenisform/-1',function(){
			$('#loader-form').hide();
		});
	});
	function edit(id)
	{
		$('#loader-form').show();
		$('#form').load('<?=site_url()?>penerimaan/jenisform/'+id,function(){
	...
	{
		$('#loader-form').show();
		$('#form').load('<?=site_url()?>penerimaan/jenisform/'+id+'/child',function(){
			$('#jenis').focus();
			$('#loader-form').hide();
		});
	}
	function hapus(id)
	{
		bootbox.confirm("<h3>Yakin Ingin Menghapus Data Jenis ini ?? </h3>", function(result) {
					...
					{
						$.ajax({
							url : '<?=site_url()?>penerimaan/jenishapus/'+id,
							success : function(a)
							{
								// alert(a);
								tampilpesan(a);

						...
						<div class="row">
							...
							<div class="col-xs-12">
							            <div style="float:right;margin-bottom:5px;">
												Tahun Ajaran
												<select name="tahun_ajaran" id="tahunajaran" class="chosen-select form-control tag-input-style col-xs-3" data-placeholder="Tahun Ajaran" onchange="reloadlaporanaktifitas()">
                                                    <?php
...
<script>
    $('#loader-data').hide();
    reloadlaporanaktifitas();
    function reloadlaporanaktifitas()
	{
        var date=$('#tahunajaran').val();
		$('#loader-data').show();
		$('#data').css({'background':'white','opacity':'0.2'});
		$('#data').load('<?=site_url()?>laporan/laporanaktifitasdata/'+date,function(){
			$('#loader-data').hide();
	...
	function laporanaktifitassekolah(date)
	{
		$('#data').css({'background':'white','opacity':'0.2'});
		$('#loader-data').show();
        $('#data').load('<?=site_url()?>laporan/laporanaktifitasdata/'+date+'/sekolah',function(){
			$('#loader-data').hide();
			$('#data').css({'background':'transparent','opacity':'1.0'});
		});
	}

						...
						<div class="row">
							...
							<div class="col-xs-12">
							            <div style="float:right;margin-bottom:5px;">
												Tahun Ajaran
												<select name="tahun_ajaran" id="tahunajaran" class="chosen-select form-control tag-input-style col-xs-3" data-placeholder="Tahun Ajaran" onchange="reloadneracalajur()">
                                                    <?php
...
<script>
    $('#loader-data').hide();
    reloadneracalajur();
    function reloadneracalajur()
	{
        var date=$('#tahunajaran').val();
		$('#loader-data').show();
		$('#data').css({'background':'white','opacity':'0.2'});
		$('#data').load('<?=site_url()?>laporan/neracalajurdata/'+date,function(){
			$('#loader-data').hide();
	...
	function neracalajurnonsekolah(date)
	{
		$('#data').css({'background':'white','opacity':'0.2'});
		$('#loader-data').show();
        $('#data').load('<?=site_url()?>laporan/neracalajurdata/'+date+'/non',function(){
			$('#loader-data').hide();
			$('#data').css({'background':'transparent','opacity':'1.0'});
		});
	}
	function neracalajursekolah(date)
	{
		$('#data').css({'background':'white','opacity':'0.2'});
		$('#loader-data').show();
        $('#data').load('<?=site_url()?>laporan/neracalajurdata/'+date+'/sekolah',function(){
			$('#loader-data').hide();
			$('#data').css({'background':'transparent','opacity':'1.0'});
		});
	}

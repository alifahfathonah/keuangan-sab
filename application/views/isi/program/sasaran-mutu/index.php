...
<script type="text/javascript">
	jQuery(function($){
		$('#loader-data').show();
		// $('#loader-form').show();
		// $('#data').load('<?=site_url()?>sasaranmutu/data',function(){
		// 	$('#loader-data').hide();
		// });
		data()
		edit(-1);
	...
	// function form(id)
	// {
	// 	$('#form').load('<?=site_url()?>sasaranmutu/form/'+id,function(){
	// 		$('#loader-form').hide();
	...
	function hapus(id)
	{
		bootbox.confirm("<h3>Yakin Ingin Menghapus Data ini ?? </h3>", function(result) {
					...
					{
						$.ajax({
							url : '<?=site_url()?>sasaranmutu/hapus/'+id,
							success : function(a)
							{
								// alert(a);
								tampilpesan(a);
								...
								$('#form').load('<?=site_url()?>sasaranmutu/form/-1',function(){
									$('#loader-form').hide();
								});
							}
						});

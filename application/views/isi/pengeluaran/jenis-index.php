...
<script type="text/javascript">
	jQuery(function($){
		$('#loader-data').show();
		$('#loader-form').show();
		$('#data').load('<?=site_url()?>pengeluaran/jenisdata',function(){
			$('#loader-data').hide();
		});
		$('#form').load('<?=site_url()?>pengeluaran/jenisform/-1',function(){
			$('#loader-form').hide();
		});
	});
	function edit(id)
	{
		$('#loader-form').show();
		$('#form').load('<?=site_url()?>pengeluaran/jenisform/'+id,function(){
			$('#loader-form').hide();
		});
	}
	...
	{
		$('#loader-form').show();
		$('#form').load('<?=site_url()?>pengeluaran/jenisform/'+id+'/child',function(){
			$('#jenis').focus();
			$('#loader-form').hide();
		});
	}

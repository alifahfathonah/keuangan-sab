...
<script type="text/javascript">
	jQuery(function($){
		$('#loader-data').show();
		$('#loader-form').show();
		$('#data').load('<?=site_url()?>config/clubdata',function(){
			$('#loader-data').hide();
		});
		$('#form').load('<?=site_url()?>config/clubform/-1',function(){
			$('#loader-form').hide();
		});
	});
	function edit(id)
	{
		$('#loader-form').show();
		$('#form').load('<?=site_url()?>config/clubform/'+id,function(){
			$('#loader-form').hide();
		});
	}
	...
	function hapus(id)
	{
		bootbox.confirm("<h3>Yakin Ingin Menghapus Data ini ?? </h3>", function(result) {
					...
					{
						$.ajax({
							url : '<?=site_url()?>config/clubhapus/'+id,
							success : function(a)
							{
								// alert(a);
								tampilpesan(a);
